...
int ModifyRows(DATA_TYPE **a, int start_row, int end_row, int step, int col, DATA_TYPE *b) {
#ifdef HIGH_VERB	
	printf("ModifyRows %d-%d with row %d and step=%d\n", start_row, end_row, col, step);
#endif	
	int i,k;	
	for (i=start_row; i<=end_row; i=i+step) {
		if (a[i][col]==0) {
			continue;
		};
		if (b[col]==0) {
...
void BroadcastRow(int j, int thread_id) {
#ifdef HIGH_VERB	
...
void WaitForRow(int j, int thread_id) {
			pthread_mutex_lock(&row_lock[j]);
...
void* ModifyMatrix(void  *thread_input_ext) {
	...
	for (j=0; j<DIM; j++) {
		if (j%step==offset) { 
			if (ModifyRows(a, j+step, DIM-1, step, j, &a[j][0])) {
				break;
			};
			if (j==0) {
				BroadcastRow(0, thread_id);
			}
		} else {
			WaitForRow(j, thread_id);
			int start_row = (j/step)*step+offset;
			if (start_row<j) {
				start_row += step;
			};
			
			if (ModifyRows(a, start_row, DIM-1, step, j, &a[j][0])) {
				break;
			};
			if ((j+1)<DIM && (j+1)%step == offset) {
				BroadcastRow(j+1, thread_id);
			};
		};
};	
// #ifdef HIGH_VERB	
...
int main() {
	...
	for (i=0; i<(NTHREADS); i++) {
		thread_inputs[i].a = a;
		thread_inputs[i].offset = i;
		thread_inputs[i].step = NTHREADS;
