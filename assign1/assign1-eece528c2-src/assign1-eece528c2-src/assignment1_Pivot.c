...
void performUpperTraingular(int pr_iSizeParameter , float ***pr_vMatrix , float **pr_vResultMatrix ){
             int l_iOuterLoop , l_iIntermediateLoop , l_iInnerLoop ;
            float l_fCompute;
	clock_t l_cBegin = clock();
       for( l_iOuterLoop = 0 ; l_iOuterLoop < pr_iSizeParameter-1 ; l_iOuterLoop++){
                    for( l_iIntermediateLoop = l_iOuterLoop+1  ; l_iIntermediateLoop < pr_iSizeParameter ; l_iIntermediateLoop++){
                            l_fCompute = (*pr_vMatrix)[l_iIntermediateLoop][l_iOuterLoop] / (*pr_vMatrix)[l_iOuterLoop][l_iOuterLoop];
    
                            for( l_iInnerLoop = l_iOuterLoop ; l_iInnerLoop < pr_iSizeParameter+RESULT_COLUMN ; l_iInnerLoop++){
...
void performPivotUpperTraingular(int pr_iSizeParameter , float ***pr_vMatrix ){
	 ...
	 int l_iOuterLoop , l_iIntermediateLoop , l_iInnerLoop , l_iFlag ;
	float l_iMax , l_fCompute;
	for( l_iOuterLoop = 0 ; l_iOuterLoop < pr_iSizeParameter-1 ; l_iOuterLoop++){
		l_iMax = (*pr_vMatrix)[l_iOuterLoop][l_iOuterLoop];
		l_iFlag = l_iOuterLoop;

		for( l_iIntermediateLoop = l_iOuterLoop+1  ; l_iIntermediateLoop < pr_iSizeParameter ; l_iIntermediateLoop++){
			if( fabs(l_iMax) < (fabs((*pr_vMatrix)[l_iIntermediateLoop][l_iOuterLoop]))){
				l_iMax = (*pr_vMatrix)[l_iIntermediateLoop][l_iOuterLoop];
				l_iFlag = l_iIntermediateLoop;
			}//if ends
		}//for loop ends

		if( l_iFlag != l_iOuterLoop)
		{
			l_iIntermediateLoop = 0 ;
			for( l_iIntermediateLoop = l_iOuterLoop ; l_iIntermediateLoop < pr_iSizeParameter+1 ; l_iIntermediateLoop++){
				l_fCompute = (*pr_vMatrix)[l_iOuterLoop][l_iIntermediateLoop];
				(*pr_vMatrix)[l_iOuterLoop][l_iIntermediateLoop] = (*pr_vMatrix)[l_iFlag][l_iIntermediateLoop];
				(*pr_vMatrix)[l_iFlag][l_iIntermediateLoop] = l_fCompute ;

			}//for ends

		}//if ends SWAPPING ENDS
	}//Outer loop ends 
}
...
void performGuassionElimination(int pr_iSizeParameter , float ***pr_vFirstMatrix ){
	...
	performUpperTraingular(pr_iSizeParameter , pr_vFirstMatrix , &g_fGuassionFinal );
