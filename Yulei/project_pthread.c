/*
@EECE528 Project - BDD Parallelization - pThreads
...
void *searchNode(void *arguments) {
	struct arg_struct *args = arguments;
	int checkNode = 0;
	
	while (checkNode < args->appMan->currentSpaceNum) {
		if (args->appMan->uniqueNodes[checkNode]->index == args->nodeIndex) {
			if (args->appMan->uniqueNodes[checkNode]->lowChild == args->left && args->appMan->uniqueNodes[checkNode]->lowChild == args->right) {
				args->appMan->found = checkNode;
			}
		}
		checkNode = checkNode + 32;
	}
	return NULL;
}
...
bddNode* addBDDs(bddTree *result, bddNode *node1, bddNode *node2, applyManager *appMan) {
	...
	if (left == right) {
		...
		if (appMan->currentSpaceNum != 0) {
			if (appMan->currentSpaceNum >= 32) {
				procNum = 32;
			} else {
				procNum = appMan->currentSpaceNum;
			}
			
			struct arg_struct args;
			args->appMan = appMan;
			args->nodeIndex = newNodeIndex;
			args->left = left;
			args->right = right;
			args->found = -1;
			...
			for (i = 0; i < procNum; i++){
				args-> procID = i;
				pthread_create(&thread[i], NULL, searchNode, (void *)&args);
			}
			for (i = 0; i < procNum; i++){
			...
			if (args->found != -1) {
				return appMan->uniqueNodes[args->found];
			}
		}
