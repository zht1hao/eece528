...
*/
__global__ void performGuassEliminationOuterLoop(float *pr_fGuassionElements, unsigned int pr_iRoWNo, unsigned int pr_iNoOfColumns)
{   
	unsigned int l_uiColumn = (blockIdx.y * (blockDim.x * blockDim.y)) + ((threadIdx.y * blockDim.x) + threadIdx.x) + pr_iRoWNo ;
	unsigned int l_uiColumnOffset = blockDim.x * blockDim.y * gridDim.y;
	float l_fCompute = pr_fGuassionElements[pr_iRoWNo * pr_iNoOfColumns + pr_iRoWNo];
	...
	unsigned int l_iLoop;
	for (l_iLoop = l_uiColumn; l_iLoop < pr_iNoOfColumns; l_iLoop += l_uiColumnOffset)
	{
		//printf("%f ",pr_fGuassionElements[pr_iRoWNo * pr_iNoOfColumns + l_iLoop]);
		pr_fGuassionElements[pr_iRoWNo * pr_iNoOfColumns + l_iLoop] /= l_fCompute;
	}
}//performGuassEliminationOuterLoop ends

__global__ void performGuassEliminationInnerLoop(float *pr_fGuassionElements, unsigned int pr_iRoWNo, unsigned int pr_iNoOfColumns)
{   
	unsigned int l_uiRow = blockIdx.y * blockDim.y + threadIdx.y + pr_iRoWNo ;
	unsigned int l_uiRowOffset = blockDim.y * gridDim.y;
	unsigned int l_uiColumn = blockIdx.x * blockDim.x + threadIdx.x;
	unsigned int l_uiColumnOffset = blockDim.x * gridDim.x;
	//Data is not visible to threads in other block 
	__shared__ float l_fMatrix[BLOCK_SIZE][BLOCK_SIZE];
	// setting the bottm right diagonal valuse to 1
	pr_fGuassionElements[pr_iRoWNo * pr_iNoOfColumns + pr_iRoWNo] = 1;
	...
	//Used blockDim.x,y gives the number of threads in a block
	//USed gridDim.x,y gives the number of blocks in a grid
	for (l_iOuterloop = l_uiRow; l_iOuterloop < pr_iNoOfColumns; l_iOuterloop += l_uiRowOffset)
	{   
		// Multiply outerloop function computed element in each row
		l_fCompute = pr_fGuassionElements[l_iOuterloop * pr_iNoOfColumns + pr_iRoWNo];
		__syncthreads();
		//Synchronizes all threads within a block
		for (l_iInnerLoop = l_uiColumn; l_iInnerLoop < pr_iNoOfColumns; l_iInnerLoop += l_uiColumnOffset)
		{   
			l_fMatrix[threadIdx.x][threadIdx.y] = pr_fGuassionElements[pr_iRoWNo * pr_iNoOfColumns + l_iInnerLoop];
			// Subtract the product from the current element
			pr_fGuassionElements[l_iOuterloop * pr_iNoOfColumns + l_iInnerLoop] -= ( l_fCompute * l_fMatrix[threadIdx.x][threadIdx.y]);
		}
	}
}
...
//	printf("\n %d", l_strGuassionMatrixGPU.th_fGuassionElements);
	for (l_iLoop = 0; l_iLoop < pr_iSizeParameter - 1; l_iLoop++ )
	{
		...
		performGuassEliminationOuterLoop <<<gpuGrid,gpuThreadBlock>>> (l_strGuassionMatrixGPU.th_fGuassionElements, l_iLoop,l_strGuassionMatrixGPU.th_uiColumns );
		cudaDeviceSynchronize();
		
		performGuassEliminationInnerLoop <<<gpuGrid,gpuThreadBlock>>> (l_strGuassionMatrixGPU.th_fGuassionElements,l_iLoop, l_strGuassionMatrixGPU.th_uiColumns );
		cudaDeviceSynchronize();
	}
